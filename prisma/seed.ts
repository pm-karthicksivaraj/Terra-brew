...
async function main() {
  ...
  try {
    ...
    const farmLand1 = await db.farmLand.create({ data: {
      ...
      fertilityStatus: 'Tốt', waterSource: 'Giếng khoan', powerSource: 'Điện lưới', isActive: true,
    }})
    const farmLand2 = await db.farmLand.create({ data: {
      ...
      powerSource: 'Điện mặt trời + Lưới', isActive: true,
    }})
    console.log('  ✅ Farm lands created')
    ...
    const cultivation1 = await db.cultivation.create({ data: {
      ...
      seedQuantity: 3000, seedPrice: 5000, seedCost: 15000000, isActive: true,
    }})
    const cultivation2 = await db.cultivation.create({ data: {
      ...
      isSeedTreated: true, seedType: 'Cải tiến', seedQuantity: 2400, seedPrice: 8000, seedCost: 19200000, isActive: true,
    }})
    console.log('  ✅ Cultivations created')
    ...
    console.log('  ✅ Hash chain blocks created')
    ...
    await db.procurementRecord.create({ data: {
      tenantId: tenant.id, collectionCentreId: cc1.id, cultivationId: cultivation1.id,
    ...
    console.log('  ✅ Procurement records created')
    ...
    console.log('  ✅ Processing job orders created')

    // Remaining records (nurseries, land prep, crop monitoring, etc.)
    await db.nursery.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, createdBy: adminUser.id, nurseryName: 'Vườn ươm Ea Tam', nurseryCode: 'NS-EATAM-001', location: 'Ea Tam, Cư Mgar, Đắk Lắk', province: 'Đắk Lắk', district: 'Cư Mgar', commune: 'Ea Tam', latitude: 12.670, longitude: 108.040, nurseryType: 'Hạt giống', capacity: 50000, currentStock: 35000, species: 'Coffea canephora', variety: 'Chari', seedSource: 'Vườn giống chứng nhận Ea Drăng', plantingDate: new Date('2024-06-01'), expectedReadyDate: new Date('2024-12-01'), germinationRate: 92.5, survivalRate: 88.0, healthStatus: 'Tốt', notes: 'Cây giống Robusta Chari thế hệ F1', isActive: true } })
    await db.nursery.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, createdBy: adminUser.id, nurseryName: 'Vườn ươm Hữu cơ Ea Drăng', nurseryCode: 'NS-EADRANG-001', location: 'Ea Drăng, Cư Mgar, Đắk Lắk', province: 'Đắk Lắk', district: 'Cư Mgar', commune: 'Ea Drăng', latitude: 12.694, longitude: 108.057, nurseryType: 'Cây giống', capacity: 30000, currentStock: 18000, species: 'Coffea arabica', variety: 'Catimor', seedSource: 'Viện Khoa học Kỹ thuật Nông lâm nghiệp Tây Nguyên', plantingDate: new Date('2024-07-15'), expectedReadyDate: new Date('2025-01-15'), germinationRate: 89.0, survivalRate: 85.5, healthStatus: 'Khá', notes: 'Cây giống Arabica Catimor', isActive: true } })

    await db.landPreparation.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, createdBy: adminUser.id, preparationDate: new Date('2024-04-10'), preparationType: 'Làm đất vụ mới', method: 'Cày xới thủ công + hữu cơ', equipmentUsed: 'Cuốc, xẻng, máy cày mini', laborCount: 4, laborCost: 2400000, materialsUsed: 'Phân chuồng 2 tấn, vôi bột 50kg', materialCost: 3500000, totalCost: 5900000, soilPhBefore: 5.2, soilPhAfter: 5.8, organicMatterPct: 3.8, notes: 'Đã bón lót phân chuồng ủ hoai mục', isActive: true } })
    await db.landPreparation.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, createdBy: adminUser.id, preparationDate: new Date('2024-03-25'), preparationType: 'Bổ sung dinh dưỡng', method: 'Bón hữu cơ + che phủ', equipmentUsed: 'Xe cút kút, cuốc', laborCount: 3, laborCost: 1800000, materialsUsed: 'Phân gà ủ 1.5 tấn, mùn cọ 500kg', materialCost: 2800000, totalCost: 4600000, soilPhBefore: 5.5, soilPhAfter: 6.0, organicMatterPct: 4.2, notes: 'Bổ sung hữu cơ theo tiêu chuẩn UTZ', isActive: true } })

    await db.cropMonitoring.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, cultivationId: cultivation1.id, createdBy: adminUser.id, monitoringDate: new Date('2024-09-15'), monitoringType: 'Kiểm tra định kỳ', growthStage: 'Phát triển trái', plantHeight: 2.1, canopyDiameter: 1.5, leafColor: 'Xanh đậm', healthScore: 88.0, pestPressure: 'Thấp', diseaseSymptoms: 'Không phát hiện', weatherCondition: 'Nắng xen mưa', temperature: 26.5, rainfall: 180, humidity: 78.0, soilMoisture: 42.0, alertTriggered: false, notes: 'Cây phát triển tốt', isActive: true } })
    await db.cropMonitoring.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, cultivationId: cultivation2.id, createdBy: adminUser.id, monitoringDate: new Date('2024-09-20'), monitoringType: 'Kiểm tra sau mưa', growthStage: 'Chín trái', plantHeight: 1.8, canopyDiameter: 1.3, leafColor: 'Xanh vàng nhẹ', healthScore: 72.0, pestPressure: 'Trung bình', diseaseSymptoms: 'Gỉ sắt nhẹ', weatherCondition: 'Mưa liên tục', temperature: 23.0, rainfall: 350, humidity: 88.0, soilMoisture: 65.0, alertTriggered: true, alertType: 'Bệnh nấm', alertSeverity: 'Trung bình', remedialAction: 'Phun thuốc BIO-FOX', notes: 'Gỉ sắt phát triển do ẩm độ cao', isActive: true } })

    await db.fertilizerApplication.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, cultivationId: cultivation1.id, createdBy: adminUser.id, applicationDate: new Date('2024-08-05'), fertilizerType: 'Hữu cơ', fertilizerName: 'Phân chuồng ủ hoai', nutrientContent: 'NPK 2-1-2', applicationRate: 2.0, unit: 'tấn/ha', totalQuantity: 4.0, applicationMethod: 'Bón gốc', costPerUnit: 800000, totalCost: 3200000, weatherAtApplication: 'Nắng nhẹ', appliedBy: 'Nguyễn Văn Thanh', isOrganic: true, certificationNumber: 'ORG-DL-2024-0456', notes: 'Bón thúc trái', isActive: true } })
    await db.fertilizerApplication.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, cultivationId: cultivation2.id, createdBy: adminUser.id, applicationDate: new Date('2024-07-20'), fertilizerType: 'Hữu cơ sinh học', fertilizerName: 'Trichoderma + Phân gà ủ', nutrientContent: 'NPK 3-2-3', applicationRate: 1.5, unit: 'tấn/ha', totalQuantity: 2.25, applicationMethod: 'Bón gốc + xịt lá', costPerUnit: 1200000, totalCost: 2700000, weatherAtApplication: 'Trời mát', appliedBy: 'Trần Thị Hoa', isOrganic: true, certificationNumber: 'ORG-DL-2024-0457', notes: 'Phun Trichoderma phòng bệnh', isActive: true } })

    await db.pestDiseaseManagement.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, cultivationId: cultivation2.id, createdBy: adminUser.id, detectionDate: new Date('2024-09-22'), pestOrDisease: 'Gỉ sắt (Coffee Leaf Rust)', type: 'Bệnh nấm', severity: 'Trung bình', affectedArea: 0.6, affectedTrees: 180, symptoms: 'Đốm vàng cam dưới lá', treatmentMethod: 'Sinh học', treatmentProduct: 'BIO-FOX', dosage: '5g/lít', applicationDate: new Date('2024-09-24'), followUpDate: new Date('2024-10-08'), outcome: 'Đang theo dõi', cost: 850000, preventionMeasures: 'Tỉa cành thông thoáng', notes: 'Phun lần 1 ngày 24/9', isActive: true } })
    await db.pestDiseaseManagement.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, cultivationId: cultivation1.id, createdBy: adminUser.id, detectionDate: new Date('2024-08-15'), pestOrDisease: 'Rệp sáp bột hồng', type: 'Sâu bệnh', severity: 'Thấp', affectedArea: 0.2, affectedTrees: 35, symptoms: 'Túi sáp trắng ở kẽ cành', treatmentMethod: 'Sinh học', treatmentProduct: 'Cryptolaemus montrouzieri', dosage: '50 bọ rệp/100 cây', applicationDate: new Date('2024-08-18'), followUpDate: new Date('2024-09-01'), outcome: 'Kiểm soát tốt', cost: 450000, preventionMeasures: 'Bảo tồn kiến vàng', notes: 'Bọ rệp đối kháng hiệu quả tốt', isActive: true } })

    await db.certAssessment.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, createdBy: adminUser.id, assessmentId: 'CERT-2024-ORG-001', assessmentDate: new Date('2024-06-15'), certificationStandard: 'Organic', certifyingBody: 'Control Union Việt Nam', assessmentType: 'Tái chứng nhận', scope: 'Sản xuất hữu cơ 2.5 ha', status: 'Đạt', score: 92.0, maxScore: 100.0, findings: 'Tuân thủ tốt quy chuẩn hữu cơ', validFrom: new Date('2024-07-01'), validUntil: new Date('2025-06-30'), certificateNumber: 'ORG-VN-2024-CU0583', isActive: true } })
    await db.certAssessment.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, createdBy: adminUser.id, assessmentId: 'CERT-2024-FT-002', assessmentDate: new Date('2024-05-20'), certificationStandard: 'Fair Trade', certifyingBody: 'FLO-CERT GmbH', assessmentType: 'Chứng nhận mới', scope: 'Thương mại công bằng 1.8 ha', status: 'Đạt', score: 87.5, maxScore: 100.0, findings: 'Đáp ứng tiêu chuẩn thương mại công bằng', validFrom: new Date('2024-06-01'), validUntil: new Date('2025-05-31'), certificateNumber: 'FT-VN-2024-ED0217', isActive: true } })

    await db.coffeeInspection.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, farmLandId: farmLand1.id, batchId: batchId1, createdBy: adminUser.id, inspectionId: 'INSP-2024-RB-001', inspectionDate: new Date('2024-11-15'), inspectorName: 'Nguyễn Thị Lan', inspectorCertNo: 'QC-VN-2024-0142', inspectionType: 'Kiểm định chất lượng đầu vào', inspectionStandard: 'TCVN 4193:2014', sampleSize: 0.3, moistureContent: 10.8, defectCount: 8.0, foreignMatter: 0.05, screenSize: 'Screen 16+', cupScore: 82.5, overallGrade: 'Grade 1 - Xuất khẩu', passFail: 'Pass', isActive: true } })
    await db.coffeeInspection.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, farmLandId: farmLand2.id, batchId: batchId2, createdBy: adminUser.id, inspectionId: 'INSP-2024-AR-001', inspectionDate: new Date('2024-10-25'), inspectorName: 'Nguyễn Thị Lan', inspectorCertNo: 'QC-VN-2024-0142', inspectionType: 'Kiểm định chất lượng đặc sản', inspectionStandard: 'SCA Protocol', sampleSize: 0.35, moistureContent: 11.2, defectCount: 3.0, foreignMatter: 0.02, screenSize: 'Screen 15+', cupScore: 85.0, overallGrade: 'Specialty Grade', passFail: 'Pass', isActive: true } })

    await db.smartContract.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, buyerId: 'BUYER-INT-001', createdBy: adminUser.id, contractId: 'SC-2024-VN-RB-001', contractType: 'Hợp đồng mua bán', title: 'Hợp đồng cung ứng Robusta 2024-2025', partyA: 'HTX Ea Tam', partyB: 'Tokyo Coffee Trading', quantityKg: 5000, pricePerKg: 55000, totalValue: 275000000, currency: 'VND', deliveryDate: new Date('2025-01-15'), status: 'Đang thực hiện', signedByA: true, signedByB: true, signedDateA: new Date('2024-10-01'), signedDateB: new Date('2024-10-05'), effectiveDate: new Date('2024-10-10'), expiryDate: new Date('2025-03-31'), isActive: true } })
    await db.smartContract.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, buyerId: 'BUYER-INT-002', createdBy: adminUser.id, contractId: 'SC-2024-VN-AR-002', contractType: 'Hợp đồng đặc sản', title: 'Hợp đồng cung ứng Arabica đặc sản 2024', partyA: 'Nông trại Ea Drăng', partyB: 'Seoul Specialty Roasters', quantityKg: 2000, pricePerKg: 120000, totalValue: 240000000, currency: 'VND', deliveryDate: new Date('2024-12-20'), status: 'Chờ ký bên A', signedByA: false, signedByB: true, signedDateB: new Date('2024-11-01'), effectiveDate: new Date('2024-11-15'), expiryDate: new Date('2025-02-28'), isActive: true } })

    await db.marketplaceListing.create({ data: { tenantId: tenant.id, farmerId: farmer1.id, createdBy: adminUser.id, listingId: 'MKT-2024-RB-001', title: 'Cà phê Robusta Chari Grade 1 - Hữu cơ', description: 'Robusta Chari Grade 1 hữu cơ vụ 2024', coffeeType: 'Robusta', coffeeVariety: 'Chari', grade: 'Grade 1', quantityKg: 3000, pricePerKg: 55000, totalValue: 165000000, currency: 'VND', origin: 'Ea Tam, Đắk Lắk', processingMethod: 'Rửa', cupScore: 82.5, certifications: 'Organic, UTZ', harvestYear: '2024', listingStatus: 'Đang bán', listingDate: new Date('2024-11-20'), isActive: true } })
    await db.marketplaceListing.create({ data: { tenantId: tenant.id, farmerId: farmer2.id, createdBy: adminUser.id, listingId: 'MKT-2024-AR-001', title: 'Arabica Catimor Natural Specialty', description: 'Arabica Catimor đặc sản, cup score 85', coffeeType: 'Arabica', coffeeVariety: 'Catimor', grade: 'Specialty', quantityKg: 1500, pricePerKg: 120000, totalValue: 180000000, currency: 'VND', origin: 'Ea Drăng, Đắk Lắk', processingMethod: 'Tự nhiên', cupScore: 85.0, certifications: 'Organic, Fair Trade', harvestYear: '2024', listingStatus: 'Đang bán', listingDate: new Date('2024-10-28'), isActive: true } })

    await db.processingStageRecord.create({ data: { tenantId: tenant.id, jobOrderId: jobOrder1.id, stageType: 'Phân loại & Làm sạch', stageDate: new Date('2024-11-13'), inputWeight: 3200, outputWeight: 3185, durationMinutes: 120, temperature: 25.0, humidity: 72.0, machineUsed: 'Máy phân loại quang học', operatorName: 'Võ Minh Trí', qualityCheckPassed: true, notes: 'Loại bỏ quả đen, quả xanh', isActive: true } })
    await db.processingStageRecord.create({ data: { tenantId: tenant.id, jobOrderId: jobOrder1.id, stageType: 'Bóc vỏ & Lên men', stageDate: new Date('2024-11-14'), inputWeight: 3185, outputWeight: 850, durationMinutes: 1440, temperature: 28.0, humidity: 80.0, machineUsed: 'Máy bóc vỏ Penagos 2500', operatorName: 'Võ Minh Trí', qualityCheckPassed: true, notes: 'Lên men ướt 24 giờ', isActive: true } })
    await db.processingStageRecord.create({ data: { tenantId: tenant.id, jobOrderId: jobOrder1.id, stageType: 'Sấy', stageDate: new Date('2024-11-15'), inputWeight: 850, outputWeight: 660, durationMinutes: 20160, temperature: 35.0, humidity: 45.0, machineUsed: 'African bed + nhà kính', operatorName: 'Lê Văn Hùng', qualityCheckPassed: true, notes: 'Sấy 14 ngày, ẩm 10.8%', isActive: true } })
    await db.processingStageRecord.create({ data: { tenantId: tenant.id, jobOrderId: jobOrder1.id, stageType: 'Bóc lụa & Phân loại cuối', stageDate: new Date('2024-11-29'), inputWeight: 660, outputWeight: 640, durationMinutes: 240, temperature: 22.0, humidity: 55.0, machineUsed: 'Máy bóc lụa CCI', operatorName: 'Võ Minh Trí', qualityCheckPassed: true, notes: 'Green bean Grade 1, Screen 16+', isActive: true } })

    await db.auditLog.create({ data: { tenantId: tenant.id, userId: adminUser.id, action: 'CREATE', entity: 'Tenant', entityId: tenant.id, details: JSON.stringify({ message: 'Seeded Metrang Coffee with full demo data' }) } })
