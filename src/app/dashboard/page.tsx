...
import {
  ...
  BarChart3, PieChart as PieChartIcon, LogOut, Globe,
  Loader2,
} from 'lucide-react'
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-coffee-200/50">
        <div className="flex items-center justify-between px-4 md:px-8 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs text-coffee-600">
              <span className="font-medium">{session.user?.name}</span>
              <Badge variant="outline" className="text-[10px] border-coffee-300">{session.user?.role}</Badge>
            </div>
        ...
        {/* Navigation tabs */}
        <div className="px-4 md:px-8 pb-2 flex gap-1 overflow-x-auto">
          {[
            { key: 'dashboard', label: t('Bảng điều khiển', 'Dashboard'), icon: BarChart3, href: '/dashboard' },
            { key: 'farmers', label: t('Nông dân', 'Farmers'), icon: Users, href: '/farmers' },
          ].map(tab => (
            <Button
              key={tab.key}
              variant={tab.key === 'dashboard' ? 'secondary' : 'ghost'}
              size="sm"
              className={`gap-1.5 text-xs ${tab.key === 'dashboard' ? 'bg-coffee-100 text-coffee-800' : 'text-coffee-500'}`}
              onClick={() => { if (tab.key !== 'dashboard') router.push(tab.href) }}
            >
