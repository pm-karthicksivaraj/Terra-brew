...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [
      ...
      db.processingStageRecord.count({ where: { tenantId } }),
    ...
    // Aggregates
    const [avgScore, avgCupScore, totalPurchaseAgg, avgPriceAgg, totalNetWeightAgg, totalLandAreaAgg] = await Promise.all([
    ...
    const [farmersPerProvince, cultivationsByCrop, processingByStage, certByType] = await Promise.all([
      ...
      db.processingStageRecord.groupBy({ by: ['stageType'], where: { tenantId }, _count: { stageType: true }, orderBy: { _count: { stageType: 'desc' } } }),
