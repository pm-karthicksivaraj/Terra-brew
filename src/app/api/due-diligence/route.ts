...
export async function GET(req: Request) {
  ...
  try {
    ...
    const operatorEori = url.searchParams.get('operatorEori') || undefined

    const where: any = { tenantId, isActive: true }
    if (status) where.status = status
    if (operatorEori) where.operatorEori = operatorEori
    if (search) {
      where.OR = [
        { ddsId: { contains: search } },
        { operatorName: { contains: search } },
        { tracesRef: { contains: search } },
      ]
    }
