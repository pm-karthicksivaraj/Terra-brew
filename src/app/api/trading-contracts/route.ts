...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const counterpartyFilter = url.searchParams.get('counterpartyId') || undefined
    ...
    if (counterpartyFilter) where.counterpartyId = counterpartyFilter
    ...
    const [records, total] = await Promise.all([
      db.tradingContract.findMany({
        ...
        take: pageSize,
        include: { counterparty: { select: { id: true, companyName: true } } },
      }),
