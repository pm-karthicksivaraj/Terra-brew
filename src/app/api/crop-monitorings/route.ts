...
export async function GET(req: Request) {
  ...
  const authError = requireTenantAccess(user, 'crop-monitorings', 'read')
  ...
  try {
    ...
    if (idParam) {
      const item = await db.cropMonitoring.findFirst({
      ...
      if (!item) return apiError('Crop monitoring not found', 404)
...
export async function POST(req: Request) {
  const user = await getAuthUser(req)
  const authError = requireTenantAccess(user, 'crop-monitorings', 'create')
  if (authError) return authError
...
export async function PUT(req: Request) {
  const user = await getAuthUser(req)
  const authError = requireTenantAccess(user, 'crop-monitorings', 'update')
  if (authError) return authError
  ...
  try {
    ...
    const existing = await db.cropMonitoring.findFirst({ where: { id, tenantId: user!.tenantId!, isActive: true } })
    if (!existing) return apiError('Crop monitoring not found', 404)

    const item = await db.cropMonitoring.update({
      where: { id },
      data,
    })
...
export async function DELETE(req: Request) {
  const user = await getAuthUser(req)
  const authError = requireTenantAccess(user, 'crop-monitorings', 'delete')
  if (authError) return authError
