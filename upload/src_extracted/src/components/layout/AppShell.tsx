...
import {
  ...
  Droplets, Sun, CircleDot, Filter, Layers, Flame, Gauge
} from 'lucide-react'
...
interface NavGroup {
  label: string
  icon: React.ElementType
  color: string        // tailwind color class for the group header icon
  children: NavChild[]
}
...
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    children: [
      ...
      { key: 'farmlands', label: 'Farm Lands', icon: MapPin },
      { key: 'cultivations', label: 'Cultivations', icon: Sprout },
    ],
  },
  {
    label: 'Pre-Harvest',
  ...
  },
  {
    ...
    children: [
      { key: 'harvest-traceabilities', label: 'Harvest Records', icon: Wheat },
    ],
  },
  {
    label: 'Procurement',
  ...
  },
  {
    ...
    color: 'text-cyan-600',
    children: [
      { key: 'qr-scan', label: 'QR Scanner', icon: QrCode },
      { key: 'qr-label', label: 'Print Labels', icon: Printer },
  ...
  },
  {
    ...
    color: 'text-rose-600',
    children: [
      { key: 'admin-reports', label: 'Admin Reports', icon: BarChart3 },
      { key: 'credit-score', label: 'Credit Score', icon: CreditCard },
    ],
  },
]

// Helper: check if current view belongs to a group
...
function NavGroupItem({ group, currentView, onNavigate, onClose }: {
  group: NavGroup
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      ...
      {/* AnimatePresence for smooth height/opacity collapse */}
...
// ======== STANDALONE DATABASE ICON (shadcn doesn't have one) ========
...
export function Sidebar() {
  const { currentView, selectedModule, setCurrentView, sidebarOpen, setSidebarOpen, logout } = useAppStore()

  if (!selectedModule || currentView === 'module-select' || currentView === 'login' || currentView === 'register') return null

  const role = useAppStore.getState().currentUser?.role || 'farmer'
  ...
  // Farmer role only sees dashboard & credit score — skip groups entirely
  if (role === 'farmer') {
    return (
      <>
        {/* Mobile overlay with AnimatePresence for smooth fade */}
        <AnimatePresence>
          {sidebarOpen && (
            <motion.div
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between p-4 border-b">
              <ScaleIn className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="font-bold text-sm text-foreground">{selectedModule.name}</h2>
...
const VIEW_TITLES: Partial<Record<ViewName, string>> = {
  ...
  'ps-cleaning-washing-form': 'Cleaning & Washing Form',
  'ps-depulping-fermentation': '2. Depulping & Fermentation',
  'ps-depulping-fermentation-form': 'Depulping & Fermentation Form',
  'ps-drying-hulling': '3. Drying & Hulling',
  'ps-drying-hulling-form': 'Drying & Hulling Form',
  'ps-grading-sorting': '4. Grading & Sorting',
  'ps-grading-sorting-form': 'Grading & Sorting Form',
  'ps-roasting-blending': '5. Roasting & Blending',
  'ps-roasting-blending-form': 'Roasting & Blending Form',
  'ps-grinding-packaging': '6. Grinding & Packaging',
  'ps-grinding-packaging-form': 'Grinding & Packaging Form',
  'ps-quality-control': '7. Quality Control',
  'ps-quality-control-form': 'Quality Control Form',
  'qr-scan': 'QR Scanner',
...
export function Header() {
  const { selectedModule, currentUser, currentView, setSidebarOpen, setCurrentView } = useAppStore()
  ...
  const quickActions = [
    { icon: Users, label: 'Add Farmer', view: 'farmer-form' as const },
    { icon: Wheat, label: 'New Harvest', view: 'harvest-trace-form' as const },
    { icon: FileText, label: 'Add Procurement', view: 'procurement-record-form' as const },
    { icon: QrCode, label: 'Trace Batch', view: 'traceability' as const },
  ]
